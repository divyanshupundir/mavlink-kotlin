 ...
 * @param consumedFuel Consumed fuel (measured). This value should not be inferred: if not measured set to NaN. NaN: field not provided.
 * @param remainingFuel Remaining fuel until empty (measured). The value should not be inferred: if not measured set to NaN. NaN: field not provided.
 * @param percentRemaining Percentage of remaining fuel, relative to full. Values: [0-100], UINT8_MAX: field not provided.
 * units = %
 * @param flowRate Positive value when emptying/using, and negative if filling/replacing. NaN: field not provided.
...
public data class FuelStatus(
  ...
  public companion object : MavMessage.MavCompanion<FuelStatus> {
    ...
    public operator fun invoke(builderAction: Builder.() -> Unit): FuelStatus = Builder().apply(builderAction).build()
  }
