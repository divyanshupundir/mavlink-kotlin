 ...
 * units = %
 * @param resultParam2 Additional result information. Can be set with a command-specific enum containing command-specific error reasons for why the command might be denied. If used, the associated enum must be documented in the corresponding MAV_CMD (this enum should have a 0 value to indicate "unused" or "unknown").
 * @param targetSystem System ID of the target recipient. This is the ID of the system that sent the command for which this COMMAND_ACK is an acknowledgement.
 * @param targetComponent Component ID of the target recipient. This is the ID of the system that sent the command for which this COMMAND_ACK is an acknowledgement.
 */
@GeneratedMavMessage(
  id = 77u,
  crcExtra = -113,
...
public data class CommandAck(
  ...
  public companion object : MavMessage.MavCompanion<CommandAck> {
    ...
    public operator fun invoke(builderAction: Builder.() -> Unit): CommandAck = Builder().apply(builderAction).build()
  }
