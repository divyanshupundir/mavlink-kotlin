 ...
 * @param firmwareVersion Version of the gimbal firmware, encoded as: `(Dev & 0xff) << 24 + (Patch & 0xff) << 16 + (Minor & 0xff) << 8 + (Major & 0xff)`.
 * @param hardwareVersion Version of the gimbal hardware, encoded as: `(Dev & 0xff) << 24 + (Patch & 0xff) << 16 + (Minor & 0xff) << 8 + (Major & 0xff)`.
 ...
 * @param rollMin Minimum hardware roll angle (positive: rolling to the right, negative: rolling to the left). NAN if unknown.
 * units = rad
 * @param rollMax Maximum hardware roll angle (positive: rolling to the right, negative: rolling to the left). NAN if unknown.
 * units = rad
 ...
 * units = rad
 * @param yawMin Minimum hardware yaw angle (positive: to the right, negative: to the left). NAN if unknown.
 * units = rad
 * @param yawMax Maximum hardware yaw angle (positive: to the right, negative: to the left). NAN if unknown.
 * units = rad
 * @param gimbalDeviceId This field is to be used if the gimbal manager and the gimbal device are the same component and hence have the same component ID. This field is then set to a number between 1-6. If the component ID is separate, this field is not required and must be set to 0.
 * @param capFlags2 Extended bitmap of gimbal capability flags (32 bit). For backwards compatibility, the lower 16 bits should also be set in cap_flags. Ground stations should prefer this field if non-zero.
 */
@GeneratedMavMessage(
  id = 283u,
  crcExtra = 74,
...
public data class GimbalDeviceInformation(
  ...
  public val capFlags2: MavBitmaskValue<GimbalDeviceCapFlags> = MavBitmaskValue.fromValue(0u),
  ...
  override fun serializeV2(): ByteArray {
    ...
    encoder.encodeBitmaskValue(capFlags2.value, 4)
  ...
  public companion object : MavMessage.MavCompanion<GimbalDeviceInformation> {
    ...
    override fun deserialize(bytes: ByteArray): GimbalDeviceInformation {
      ...
      val capFlags2 = decoder.safeDecodeBitmaskValue(4).let { value ->
        val flags = GimbalDeviceCapFlags.getFlagsFromValue(value)
        if (flags.isNotEmpty()) MavBitmaskValue.of(flags) else MavBitmaskValue.fromValue(value)
      }
    ...
    public operator fun invoke(builderAction: Builder.() -> Unit): GimbalDeviceInformation = Builder().apply(builderAction).build()
  }
  ...
  public class Builder {
    ...
    public var capFlags2: MavBitmaskValue<GimbalDeviceCapFlags> = MavBitmaskValue.fromValue(0u)
